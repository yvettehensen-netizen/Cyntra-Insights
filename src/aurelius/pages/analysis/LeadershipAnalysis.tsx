...
export default function LeadershipAnalysisPage() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("Leiderschapsanalyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Leiderschap Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Leiderschap wordt geanalyseerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Beschrijf mandaat, besluitvorming, eigenaarschap, conflictvermijding, accountability, voorbeeldgedrag en waar leiderschap zichtbaar faalt of ontwijkt."
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Leiderschap Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
