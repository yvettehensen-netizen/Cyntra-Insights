...
export default function OnderstroomAnalysis() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("Onderstroomanalyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Onderstroom Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Onderstroom wordt geanalyseerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Beschrijf vermeden conflicten, informele macht, ongeschreven regels, cynisme, loyaliteiten en wat er niet hardop gezegd wordt."
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Onderstroom Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
