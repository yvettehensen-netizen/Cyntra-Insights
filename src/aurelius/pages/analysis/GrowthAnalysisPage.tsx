...
export default function GrowthAnalysis() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("Growth analyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Groei & Schaling Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Groei & schaling wordt geanalyseerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Wat drijft groei nu? Wat schaalt mee? Waar breekt het bij +30–50% volume? Welke bottleneck beperkt duurzame groei? Wat wordt opgeschaald zonder rendement?"
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Groei & Schaling Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
