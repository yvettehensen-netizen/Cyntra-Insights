...
export default function TeamCultureAnalysis() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("Team & cultuur analyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Team & Cultuur Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Team & cultuur analyse wordt uitgevoerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Beschrijf spanningen, informele macht, conflictvermijding, ongeschreven regels, vertrouwen en wat men niet durft uit te spreken."
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Team & Cultuur Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
