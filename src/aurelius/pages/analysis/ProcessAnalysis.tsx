...
export default function ProcessAnalysis() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("Procesanalyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Proces & Operatie Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Processen worden geanalyseerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Beschrijf concrete workflows, knelpunten, wachttijden, rework, handoffs en waar processen niet meeschalen met groei."
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Procesanalyse"}
            </Button>
          </CardContent>
        </Card>
      )}
