...
const ACCEPTED_TYPES = [".pdf", ".txt", ".docx", ".xlsx", ".csv"];

/* =========================
   COMPONENT
========================= */

export default function FinancialStrategyAnalysis() {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const isValid = ACCEPTED_TYPES.some((ext) =>
      selected.name.toLowerCase().endsWith(ext)
    );

    if (!isValid) {
      alert("Ongeldig bestandstype. Alleen PDF, TXT, DOCX, XLSX en CSV.");
      e.target.value = "";
      return;
    }
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("Financial Strategy analyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Financiële Strategie Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Financiële strategie wordt geanalyseerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Kapitaalallocatie, investeringsstrategie, cashflow-discipline, schuldenstructuur, ROI, lange-termijn waardecreatie…"
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Financiële Strategie Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
