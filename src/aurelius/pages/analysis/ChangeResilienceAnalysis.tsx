...
export default function ChangeResilienceAnalysis() {
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Veranderkracht & Resilience Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>Veranderkrachtanalyse wordt uitgevoerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            <h1 className="text-3xl font-bold text-[#D4AF37] flex items-center gap-3">
              ...
              Veranderkracht & Resilience Analyse
            </h1>
            ...
            <textarea
              ...
              placeholder="Beschrijf hoe de organisatie omgaat met verandering, onzekerheid, weerstand, druk, leiderschap en gedragsreacties onder stress…"
            />
            ...
            >
              {loading ? "Analyseren…" : "Start Veranderkracht Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
