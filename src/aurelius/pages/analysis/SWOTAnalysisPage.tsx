...
export default function SWOTAnalysisPage() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error("SWOT analyse mislukt:", err);
    }
  };
  ...
  const downloadPDF = () => {
    if (!report) return;
    generateAureliusPDF(
      report.title ?? "Integrale SWOT Analyse",
      report
    );
  };
    ...
    <div className="min-h-screen max-w-6xl mx-auto px-6 pt-32 pb-40 text-white">
      {loading && (
        <div className="mb-6 flex items-center gap-3 text-gray-400">
          ...
          <span>SWOT-analyse wordt uitgevoerd…</span>
        </div>
      )}
        ...
        <Card className="bg-black/40 border-white/10">
          <CardContent className="space-y-6 p-6">
            ...
            <textarea
              ...
              placeholder="Beschrijf krachten, zwaktes, kansen en bedreigingen — inclusief zelfbedrog, ongetoetste aannames en strategische spanningen."
            />
            ...
            >
              {loading ? "Analyseren…" : "Start SWOT Analyse"}
            </Button>
          </CardContent>
        </Card>
      )}
